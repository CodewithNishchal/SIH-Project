...
/**
 * A responsive sidebar navigation component for the Analyst role.
 ...
 */
export default function AnalystSidebar({ isOpen, onLogout }) {

    // Define class strings for NavLink to keep the JSX clean
    const navLinkClasses = "flex items-center py-3 px-4 rounded-xl border-l-4 transition-all duration-300 mb-1";
    const activeClasses = "bg-sky-700 text-white font-semibold border-sky-400 shadow-lg shadow-sky-900/50";
    const inactiveClasses = "text-gray-300 hover:bg-sky-700/70 hover:text-white border-transparent hover:border-sky-400";

    return (
        <aside
            className={`sidebar bg-gradient-to-br from-slate-900 to-sky-900 text-gray-200 w-64 fixed inset-y-0 left-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 z-50 transition-transform duration-300 ease-in-out overflow-y-auto flex flex-col shadow-2xl`}
        >
            <div className="p-5 border-b border-sky-700/50 mb-4">
                <Link to="/users/analyst/dashboard" className="flex items-center space-x-3">
                    <LogoIcon />
            ...
            <nav className="flex-grow px-3">
                ...
                <span className="text-xs text-sky-400 uppercase font-bold px-4 mt-6 mb-2 block tracking-widest">
                    Analytics & Response
                </span>

                <NavLink to="/users/analyst/R&A" className={({ isActive }) => `${navLinkClasses} ${isActive ? activeClasses : inactiveClasses}`}>
                    <ReportsIcon />
                    Reports & Analysis
                </NavLink>
                <NavLink to="/users/analyst/allAlerts" className={({ isActive }) => `${navLinkClasses} ${isActive ? activeClasses : inactiveClasses}`}>
                    <AlertsIcon />
                    Alerts Generated
                </NavLink>
                <NavLink to="/users/analyst/historical" className={({ isActive }) => `${navLinkClasses} ${isActive ? activeClasses : inactiveClasses}`}>
                    <HistoricalDataIcon />
                    Historical Data
                </NavLink>
                ...
                <NavLink to="/users/analyst/social_feeds" className={({ isActive }) => `${navLinkClasses} ${isActive ? activeClasses : inactiveClasses}`}>
                    <SocialFeedsIcon />
                    Social Media Feeds
                </NavLink>
                <NavLink to="/users/analyst/ai_logs" className={({ isActive }) => `${navLinkClasses} ${isActive ? activeClasses : inactiveClasses}`}>
                    <AIPredictionIcon />
                    AI Prediction Logs
                </NavLink>
            </nav>
            ...
            <div className="mt-auto p-3">
                <div className="border-t border-sky-700/50 pt-2 space-y-2">
                    <NavLink
                        to="/users/analyst/settings"
                        className={({ isActive }) => `${navLinkClasses} ${isActive ? activeClasses : inactiveClasses}`}
                    >
                        <SettingsIcon />
                        Settings
                    ...
                    <button
                        onClick={onLogout}
                        className="w-full flex items-center py-3 px-4 rounded-xl text-red-300 hover:bg-red-800/80 hover:text-white border-l-4 border-transparent hover:border-red-400 transition-colors duration-200 group"
                    >
                        <LogoutIcon />
                        Logout
                    </button>
